#!/usr/bin/env python3
"""Sync nav partial into all subpage HTML files."""
...
PATTERN = re.compile(r'<div class="nav-links">.*?</div>\s*(?=<button class="mobile-menu-btn">)', re.DOTALL)


def sync_file(path: Path) -> bool:
    html = path.read_text()
    if '<div class="nav-links">' not in html:
        return False
    new_html, count = PATTERN.subn(NAV + '\n', html, count=1)
    if count:
